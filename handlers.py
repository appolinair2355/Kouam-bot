...
class TelegramHandlers:
            ...
            if 'text' in message:
                ...
                if self.card_predictor.has_completion_indicators(text):
                    ...
                    if should_predict and game_number is not None and combination is not None:
                        ...
                        # Envoyer la prédiction au canal spécifique et stocker CORRECTEMENT
                        ...
                        if sent_message_info and isinstance(sent_message_info, dict) and 'message_id' in sent_message_info:
                            # CORRECTION: Stocker pour le jeu prédit (game_number + 2)
    ...
    def _process_card_message(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # SYSTÈME DE VÉRIFICATION AMÉLIORÉ - Traitement sur TOUS les messages avec ✅
            if '✅' in text:
                logger.info(f"🔍 VÉRIFICATION MESSAGE NORMAL avec ✅: {text[:50]}...")
                ...
                if verification_result:
                    ...
                    if verification_result['type'] == 'update_message':
                        ...
                        if predicted_game in self.card_predictor.sent_predictions:
                            ...
                            if edit_success:
                                logger.info(f"✅ Message de prédiction édité pour jeu {predicted_game} depuis message normal")
                            else:
                ...
                logger.info(f"🔍 MESSAGE NORMAL sans ✅ - Pas de vérification: {text[:30]}...")
    ...
    def _process_verification_on_normal_message(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # SYSTÈME DE VÉRIFICATION AMÉLIORÉ - Vérifier sur TOUS les messages finaux
            if self.card_predictor.has_completion_indicators(text):
                logger.info(f"🎯 MESSAGE NORMAL FINALISÉ - Vérification des prédictions en attente")

                # Utiliser le système de vérification amélioré (is_edited=False)
    ...
    def _handle_announce_command(self, chat_id: int, text: str, user_id: int = None) -> None:
        ...
        try:
            ...
            target_channel = self.get_redirect_channel(-1002682552255)  # Use default source
    ...
    def get_redirect_channel(self, source_chat_id: int) -> int:
        """Get the target channel for redirection, defaults to 1002875505624"""
        ...
        # Canal par défaut corrigé pour 1002875505624
        return -1002875505624
