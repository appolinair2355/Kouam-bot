...
class CardPredictor:
    ...
    def _verify_prediction_common(self, message: str, is_edited: bool = False) -> Optional[Dict]:
        ...
        # VÉRIFICATION SÉQUENTIELLE: Seulement sur les prédictions réelles
        for predicted_game in sorted(self.predictions.keys()):
            ...
            if 0 <= verification_offset <= 3:
                ...
                logger.info(f"🔍 🔍 RECHERCHE COSTUME - Jeu {game_number}: Recherche costume {predicted_costume} dans PREMIER parenthèses (décalage {verification_offset})")
                ...
                if costume_found:
                    # SUCCÈS - Le costume prédit a été trouvé
                    ...
                    logger.info(f"🔍 ✅ SUCCÈS CONFIRMÉ - Costume {predicted_costume} trouvé dans jeu {game_number} (décalage {verification_offset})")
                    logger.info(f"🔍 📝 MISE À JOUR - '{original_message}' → '{updated_message}'")
                    logger.info(f"🔍 🛑 ARRÊT - Succès confirmé pour prédiction {predicted_game}")
                    ...
                    # Le costume n'a pas été trouvé à ce décalage - continuer
                    logger.info(f"🔍 ⏳ COSTUME ABSENT - {predicted_costume} non trouvé dans le premier parenthèses du jeu {game_number} (décalage {verification_offset})")
                    
            # Vérifier si on doit marquer comme échec après décalage +4
                ...
                logger.info(f"🔍 ❌ ÉCHEC CONFIRMÉ - Aucun succès trouvé pour {predicted_game} après 4 jeux (décalages 0-3)")
                logger.info(f"🔍 🛑 ARRÊT - Échec confirmé pour prédiction {predicted_game}")
                return {
                    'type': 'update_message',
                    'predicted_game': predicted_game,
                    'new_message': updated_message,
